...
export const TenantAccountTab = ({ 
  user, tenantRooms, profileForm, setProfileForm, profileLoading, profileSaving, profileSaveMsg, handleSaveProfile,
  passwordForm, setPasswordForm, passwordLoading, passwordMsg, showPassword, setShowPassword,
  handleUpdatePassword, onLogout
}: TenantAccountTabProps) => {

  const updateProfileField = (field: string, val: string) => {
    setProfileForm((f: any) => ({ ...f, [field]: val }));
  };
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-6">
                      <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="md:col-span-2"><InputField label="Địa chỉ thường trú" icon={MapPin} field="permanent_address" placeholder="Số nhà, đường, phường/xã, quận/huyện, tỉnh/thành" value={profileForm.permanent_address} onChange={(v: string) => updateProfileField('permanent_address', v)} /></div>
                        </div>
                      </div>
                      ...
                      <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <InputField label="Số CCCD / CMND" icon={Layers} field="id_card_number" placeholder="012345678901" value={profileForm.id_card_number} onChange={(v: string) => updateProfileField('id_card_number', v)} />
                      ...
                      <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <InputField label="Tên ngân hàng" icon={ShieldCheck} field="bank_name" placeholder="Vietcombank, MB Bank..." value={profileForm.bank_name} onChange={(v: string) => updateProfileField('bank_name', v)} />
                          <InputField label="Số tài khoản" icon={Home} field="bank_account_number" placeholder="1234567890" value={profileForm.bank_account_number} onChange={(v: string) => updateProfileField('bank_account_number', v)} />
                          <div className="md:col-span-2"><InputField label="Tên chủ tài khoản" icon={User} field="bank_account_name" placeholder="NGUYEN VAN A" value={profileForm.bank_account_name} onChange={(v: string) => updateProfileField('bank_account_name', v)} /></div>
                          <InputField label="Số Zalo" icon={Phone} field="zalo_phone" type="tel" placeholder="0901 234 567" value={profileForm.zalo_phone} onChange={(v: string) => updateProfileField('zalo_phone', v)} />
                          <InputField label="Tên người thân (khẩn cấp)" icon={Users} field="emergency_contact_name" placeholder="Nguyễn Văn B" value={profileForm.emergency_contact_name} onChange={(v: string) => updateProfileField('emergency_contact_name', v)} />
                          <div className="md:col-span-2"><InputField label="SĐT người thân (khẩn cấp)" icon={PhoneCall} field="emergency_contact_phone" type="tel" placeholder="0901 234 567" value={profileForm.emergency_contact_phone} onChange={(v: string) => updateProfileField('emergency_contact_phone', v)} /></div>
                        </div>
                      </div>
                         ...
                         <form onSubmit={handleUpdatePassword} className="flex flex-col gap-4">
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="relative">
                                ...
                                <input type={showPassword ? 'text' : 'password'} value={passwordForm.new} onChange={e => setPasswordForm((f: any) => ({ ...f, new: e.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 font-bold text-slate-700 p-4 pl-11 transition-all outline-none text-sm" placeholder="••••••••" />
                              </div>
                            </div>
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="relative">
                                <LockIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                <input type={showPassword ? 'text' : 'password'} value={passwordForm.confirm} onChange={e => setPasswordForm((f: any) => ({ ...f, confirm: e.target.value }))} className="w-full rounded-2xl border border-slate-200 bg-slate-50 font-bold text-slate-700 p-4 pl-11 transition-all outline-none text-sm" placeholder="••••••••" />
                              </div>
                            </div>
